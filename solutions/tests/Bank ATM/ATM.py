...
def main():
    ...
    # If login is successful, show the menu and let the user do things like check balance, deposit money, or withdraw money
    while True:
        choice = display_menu()
        if choice == "1":
            ...
            print("Thank you for choosing WIT International Bank. The Bank that you can always rely on.")
    ...
    while True:
        try:
            ...
            if amount > 0:
                ...
                print(f"${amount} deposited successfully. New balance: ${accounts[user]['balance']}")
                break
            else:
    ...
    while True:
        try:
            ...
            if 0 < amount <= accounts[user]["balance"]:
                accounts[user]["balance"] -= amount
                print(f"${amount} withdrawn successfully. New balance: ${accounts[user]['balance']}")
                break
            else:
                print("Insufficient balance or invalid amount! Please check and try again.")
        except ValueError:
            print("Invalid input! Please enter a valid number.")
